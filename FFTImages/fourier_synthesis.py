...
def calculate_distance_from_center(coords, center):
    ...
    return np.sqrt(
        (coords[0] - center) ** 2 + (coords[1] - center) ** 2
    )


def find_symmetric_coordinates(coords, center):
    return (center + (center - coords[0]),
            center + (center - coords[1]))
...
# Array dimensions (array is square) and center pixel
...
# including the column at the center of the array (which
# includes the center pixel)
coords_left_half = (
    (x, y) for x in range(array_size) for y in range(center+1)
)

# Sort points based on distance from center
coords_left_half = sorted(
    coords_left_half,
    key=lambda x: calculate_distance_from_center(x, center)
)
